...
`=` padding varies by model. One spelling trap when matching by path: the placeholder _entry_ model path
ends in `dictionnary` (double-n), while the _index_ model path ends in `dictionary` (single-n).
...
- **Source/default locale**: `en_US` is the translation _source_ (other-language content is translated
...
- A **grouped (PZN) variation is always created in the `en_US` locale** (under the `pzn/` folder above),
  ...
  `fr_BE`) that decide which audiences the grouped variation targets — _not_ a locale path segment. The
...
- `de_DE` can **not** have an `fr_FR`, `en_GB`, or `it_IT` variation. Producing content in a _different_
...
| **Translations** | Two distinct things. A **locale variation** is a same-language regional variant (e.g. `de_CH` of parent `de_DE`), listed in the parent's `variations` (see §5). A **translation** produces a _different-language_ fragment (e.g. `de_DE` from `en_US` source) and is tracked by `translation-project` fragments — not created through the locale-variation flow. |
...
lists the _only_ locales for which a locale variation may be created from that parent (same language,
