import { html as c, LitElement as A, nothing as m } from '/deps/lit-all.min.js';
import { css as _ } from '/deps/lit-all.min.js';
var h = _`
    :host {
...
var x = 'span[is="inline-price"][data-wcs-osi]',
    b =
        'a[is="checkout-link"][data-wcs-osi],button[is="checkout-button"][data-wcs-osi]';
var g = 'a[is="upt-link"]',
    N = `${x},${b},${g}`;
var p = 'merch-quantity-selector:change',
    s = 'merch-card-quantity:change';
var S = Object.freeze({
...
function n(d, e) {
    ...
    return function () {
        let o = this,
            i = arguments;
        clearTimeout(t), (t = setTimeout(() => d.apply(o, i), e));
    };
}
var [I, P, r, a, u, E] = [
    'ArrowLeft',
...
var l = class extends A {
    ...
    constructor() {
        super(),
            ...
            (this.handleKeyupDebounced = n(this.handleKeyup.bind(this), 500)),
            (this.debouncedQuantityUpdate = n(
    ...
    connectedCallback() {
        super.connectedCallback(),
            ...
            this.addEventListener(s, this.debouncedQuantityUpdate);
    }
    get button() {
        return this.shadowRoot.querySelector('button');
    }
    handleKeyup(e) {
        e.key === a || e.key === r || (this.handleInput(), this.sendEvent());
    }
    selectValue() {
        if (!this.closed) {
            let e = this.options[this.highlightedIndex];
            if (!e) {
                this.closed = !0;
                return;
            }
    ...
    handleKeydown(e) {
        ...
        e.composedPath().includes(this) && e.stopPropagation();
    }
    adjustInput(e, t) {
        (this.selectedValue = t),
            (e.value = t),
            (this.highlightedIndex = this.options.indexOf(t));
    }
    handleInput() {
        let e = this.shadowRoot.querySelector('.text-field-input'),
            t = e.value.replace(/\D/g, '');
        e.value = t;
        let o = parseInt(t);
        if (!isNaN(o))
            if (o > 0 && o !== this.selectedValue) {
                let i = o;
                this.maxInput && o > this.maxInput && (i = this.maxInput),
                    this.min && i < this.min && (i = this.min),
                    this.adjustInput(e, i);
            } else this.adjustInput(e, this.selectedValue || this.min || 1);
    }
    disconnectedCallback() {
        super.disconnectedCallback(),
            window.removeEventListener('mousedown', this.handleClickOutside),
            this.removeEventListener('keydown', this.boundKeydownListener),
            this.removeEventListener(s, this.debouncedQuantityUpdate);
    }
    generateOptionsArray() {
        let e = [];
        if (this.step > 0)
            for (let t = this.min; t <= this.max; t += this.step) e.push(t);
        return e;
    }
    update(e) {
        (e.has('min') ||
    ...
    adjustPopoverPlacement() {
        let e = this.shadowRoot.querySelector('.popover');
        this.closed || e.getBoundingClientRect().bottom <= window.innerHeight
            ? e.setAttribute('placement', 'bottom')
            : e.setAttribute('placement', 'top');
    }
    handleMouseEnter(e) {
        this.highlightedIndex = e;
    }
    handleMenuOption(e, t) {
        e === this.max &&
...
customElements.define('merch-quantity-select', l);
export { l as MerchQuantitySelect };
