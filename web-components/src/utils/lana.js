(function iife() {
    ...
    function isProd() {
        ...
        if (
            host.substring(host.length - 10) === '.adobe.com' &&
    ...
    function log(msg, options) {
        ...
        if (options && options.severity !== undefined) {
            if (VALID_SEVERITIES.has(options.severity)) {
                ...
                console.warn(
                    `LANA: Invalid severity '${options.severity}'. Defaulting to '${defaultSeverity}'.`,
                );
        ...
        const sampleRateParam = parseInt(
            new URL(window.location).searchParams.get('lana-sample'),
            10,
        );
        const sampleRate =
            sampleRateParam ||
            (o.errorType === 'i' ? o.implicitSampleRate : o.sampleRate);

        if (
            !w.lana.debug &&
            !w.lana.localhost &&
            sampleRate <= Math.random() * 100
        )
            return;
        ...
        const endpoint =
            !isProdDomain || !o.useProd ? o.endpointStage : o.endpoint;
        const queryParams = [
        ...
        if (!isProdDomain || w.lana.debug || w.lana.localhost)
