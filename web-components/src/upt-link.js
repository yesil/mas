...
export class UptLink extends HTMLAnchorElement {
    ...
    composePromoTermsUrl() {
        ...
        if (!osi) {
            const fragmentId = this.closest('merch-card')
                .querySelector('aem-fragment')
                .getAttribute('fragment');
            console.error(
                `Missing 'data-wcs-osi' attribute on upt-link. Fragment: ${fragmentId}`,
            );
            return;
        }
        ...
        Promise.all(promises)
            ...
            .catch((error) => {
                console.error(
                    `Could not resolve offer selectors for id: ${osi}.`,
                    error.message,
                );
            });
    }
    ...
    static createFrom(element) {
        ...
        for (const attribute of element.attributes) {
            ...
            if (
                ...
                attribute.value.includes('upt-link')
            )
